...
const ReceptionistOverview = () => {
    ...
    const cards = [
        ...
        { title: 'Waiting to Check-in', value: stats?.pending_checkins || 0, icon: <Landmark />, color: 'bg-indigo-600', sub: 'Patients at the desk' },
    ];

    return (
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="p-8">
            <h2 className="text-4xl font-black italic uppercase text-gray-800 tracking-tighter mb-2">Registration Desk</h2>
            <p className="font-bold tracking-widest text-xs text-gray-400 uppercase mb-10 pl-1 font-['Montserrat']">Check in patients and create new accounts</p>
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                <Link to="/dashboard/staff/scan" className="lg:col-span-2 group">
                    <div className="card-premium p-12 bg-gradient-to-br from-indigo-900 to-blue-900 text-white shadow-2xl shadow-indigo-900/40 relative overflow-hidden group-hover:scale-[1.005] transition-transform">
                        ...
                        <p className="text-indigo-200 font-bold uppercase tracking-widest text-xs max-w-sm mb-10 leading-relaxed shadow-sm font-['Montserrat']">Scan a patient's QR card to see their medical history and check them in.</p>
                        <div className="bg-white/10 inline-grid grid-cols-2 gap-4 border border-white/20 p-2 rounded-2xl backdrop-blur-md">
                            <div className="flex items-center gap-2 px-4 py-2 bg-indigo-500 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg">Check ID</div>
                            <div className="flex items-center gap-2 px-4 py-2 bg-white/10 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg">View History</div>
                        </div>
                    </div>
                </Link>
                ...
                <Link to="/dashboard/staff/register" className="group">
                    <div className="card-premium p-12 h-full border-2 border-dashed border-gray-200 flex flex-col items-center justify-center text-center hover:bg-gray-50/50 hover:border-indigo-200 transition-all group-hover:border-solid group-hover:bg-white shadow-none hover:shadow-2xl hover:shadow-indigo-900/5">
                        ...
                        <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mt-2 max-w-[160px] leading-relaxed font-['Montserrat']">Add a new patient and give them a health card.</p>
                    </div>
                </Link>
            </div>
        </motion.div>
