...
const ReceptionistOverview = () => {
    ...
    if (loading) return <div className="p-8 font-black uppercase tracking-widest text-gray-400">Syncing Data Flow...</div>;

    const cards = [
        { title: "Today's Admissions", value: stats?.today_appointments || 0, icon: <CalendarDays />, color: 'bg-[var(--color-primary)]', sub: 'Scheduled visits' },
        { title: 'Pending Check-ins', value: stats?.pending_checkins || 0, icon: <Landmark />, color: 'bg-indigo-600', sub: 'Waiting at desk' },
    ];
